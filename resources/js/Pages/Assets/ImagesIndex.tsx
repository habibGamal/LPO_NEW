...
const AddImages = ({ assets }: { assets: AssetsModel }) => {
    ...
    const submit = (values: any) => {
        Inertia.post(AssetsModel.insertImages(), {
            ...values,
            name: assets.name,
                            ...
                            {fields.map(({ key, name, ...restField }) => (
                                <div key={key}>
                                    <Form.Item
                                        ...
                                        name={[name, 'image']}
                                    >
                                        <AntdImgCrop aspect={16 / 9} onModalOk={(file) => form.setFieldValue(['content', name, 'image'], file)}>
...
const EditImage = ({ node, name, close }: { node: ImagesAssets, name: AssetsName, close: () => void }) => {
    ...
    const submit = (values: any) => {
        ...
        Inertia.post(AssetsModel.editImages(), {
            ...values,
            name: name,
    ...
    return (
        ...
        >
            ...
            <Form.Item
                name={['content', 'image']}
            >
                <AntdImgCrop aspect={16 / 9} onModalOk={(file) => form.setFieldValue(['content', 'image'], file)}>
...
export default function ImagesIndex({ assetsDB }: { assetsDB: AssetsDB }) {
    ...
    const [{ isOpen, data: modalData }, open, close, setModalData] = useModal<{ node: ImagesAssets, name: AssetsName }>();
    const assets = new AssetsModel(assetsDB);
    const removeImage = (node: ImagesAssets) => Inertia.post(AssetsModel.removeImage(), { node_id: node.id, name: assets.name, assets_type: assets.assetsType })
    return (
        <Layout>
            <>
                <Admin>
                ...
                <Carousel className="rounded shadow p-4 bg-gray-100 my-4" dots={{ className: 'text-black' }}>

                    {
                        assets.images.map(
                            node =>
                                <div key={node.id} className="w-full relative">
                                    <img className='w-full aspect-video object-cover' src={node.path} />
                                    ...
                                    <Admin>
                                        <div className="absolute bottom-8 left-4 bg-gray-50 rounded px-4 py-2 flex gap-4">
                                            <Button onClick={() => setModalData({ node, name: assets.name })} type='dashed' size='small' icon={<FontAwesomeIcon icon={faEdit} />} />
                                            <Button onClick={() => removeImage(node)} danger size='small' icon={<FontAwesomeIcon icon={faTrash} />} />
                                        </div>
                                    </Admin>
                                </div>
                        )
                ...
                <Modal title="Edit asset" open={isOpen} onCancel={close} footer={null} destroyOnClose>
                    {
                        modalData &&
                        <EditImage node={modalData.node} name={modalData.name} close={close} />
                    }
                </Modal>
            </>
        </Layout>
