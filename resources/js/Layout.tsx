...
const DismissableLayer = ({ hideNav, navState }: { hideNav: () => void, navState: boolean }) => (
    <div onClick={hideNav} className={`fixed top-0 left-0 w-screen ${navState ? 'h-screen' : 'h-0'} bg-black opacity-40 z-40`}></div>
)
        ...
        <nav className="bg-ov-white ">
            <div className="container flex items-center justify-between py-2">
                ...
                <div className="flex gap-4 items-center">
                    ...
                    <ul className={`${navState ? 'translate-x-0 opacity-100' : t('-translate-x-full opacity-0', 'translate-x-full opacity-0')}  ${t('left-0', 'right-0')} flex nav transition-all shadow 2xl:translate-x-0 2xl:opacity-100 2xl:shadow-none 2xl:items-center items-start 2xl:flex gap-6 font-bold`}>
                    ...
                    <Button key={0} className={`fixed z-[60] top-2 ${t('left-6', 'right-6')} ${navState || 'hidden'}`} onClick={() => setNavState(false)} icon={<FontAwesomeIcon size='2x' icon={faClose} />} />
                    <div id="navbars" className="flex items-center gap-4">
                        <Button onClick={toggleLanguage} icon={<FontAwesomeIcon size='2x' icon={faEarth} />} />
                        <button id='n1avbars' className='2xl:hidden' onClick={() => setNavState(!navState)}>
...
const ToTopBtn = ({ innerRef }: { innerRef: React.RefObject<HTMLButtonElement> }) => (
    <button
        onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
        ref={innerRef}
        className="btn to-top hidden"><FontAwesomeIcon icon={faArrowUp} />
    </button>
)
